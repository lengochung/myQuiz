<?php
    ...
    function students () {
        ...
        // Thiết lập user gv về check 0 kẻo bị xóa oan uổng
        $conn->load('users')->upd_column_value('role', 2, 'checked', 0);
        ...
        if(isset($_POST['add'])) {
            ...
            Promise::resolve($conn)
                  ->then( function ($result) {
                    ...
                    $result->load("users")->insert_into($_POST['uid'], $_POST['name'], $_POST["email"], md5("12345678"), 3, 0, null, "default.jpg", 0);
                    $name = $_POST['name'];
                    send_message("<a href=''><small><b>$name</b></small></a> đã được thêm vào hệ thống!", "success");
        ...
        if(isset($_POST['submit'])):
            ...
            $promise
                  ...
                  ->then( function ($result) { // Kiểm tra đuôi file
                    ...
                    foreach ($result as $key => $row) {
                        $GLOBALS['users']->insert_into('Null', $row['A'], $row['B'], md5('12345678'), 3, 0, '', 'default.jpg', 0);
                    }
        ...
        if(isset($_POST['del_check'])) {
            ...
            foreach ($arr_del as $key => $user) {
                $conn->load("users_questions_assigns")->delete_column_value('uid', $user['uid']);
                $conn->load("users_assigns")->delete_column_value('uid', $user['uid']);
                $conn->load("class_detail")->delete_column_value('uid', $user['uid']);
                $conn->load("users")->delete_column_value('uid', $user['uid']);
            }

            send_message("Đã xóa " . count($arr_del) . " sinh viên khỏi hệ thống!", "success");
        }
        ...
        if(isset($_SESSION['sort'])) {
            $list = $conn->load("users")->get_column_value_i_limit_asc('role', 3, $_SESSION['sort'], $index, $limit);
        } else {
            $list = $conn->load("users")->get_column_value_i_limit_asc('role', 3, 'uid', $index, $limit);
        }

        // Render
        render_admin('students', [
            'title' => 'Admin', //$title
            'active' => 2,      //$active
            "ids" => $ids,

            'list' => $list,
            'num_logs' => ceil($conn->load("users")->count_column_value('role', 3)/$limit),
            'log' => $log
        ]);
    }